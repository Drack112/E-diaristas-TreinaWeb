...
from api.services import diaria_service
from api.serializers import pagamento_diaria_serializer
from api.services.pagamento_diaria_service import realizar_pagamento
from api.permissions import cliente_permission


class PagamentoDiaria(APIView):
    permission_classes = [
        cliente_permission.ClientePermission,
    ]
    ...
    def post(self, request, diaria_id, format=None):
        ...
        if serializer_pagamento.is_valid():
            ...
            if diaria.status == 1:
                ...
                return Response(
                    {"Diária paga com sucesso"}, status=status_http.HTTP_200_OK
                )
            return Response(
                {"Não é possível pagar essa diária"},
                status=status_http.HTTP_400_BAD_REQUEST,
            )
        return Response(
            serializer_pagamento.errors, status=status_http.HTTP_400_BAD_REQUEST
        )
