...
from api.services.diaria_service import atualizar_status_diaria, listar_diaria_id
...
def cancelar_pagamento(diaria_id, penalidade):
    ...
    cancelamento = pagarme.transaction.refund(
        pagamento.transacao_id, {"amount": float(valor * 100)}
    )
