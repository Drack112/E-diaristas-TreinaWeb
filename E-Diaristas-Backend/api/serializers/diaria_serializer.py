...
from api.models import Diaria, Usuario
from api.services.cidades_atendimento_service import (
    verificar_disponibilidade_cidade,
    buscar_cidade_ibge,
)
from api.services.avaliacao_diaria_service import verificar_avaliacao_usuario
...
class DiariaSerializer(serializers.ModelSerializer):
    ...
    valor_comissao = serializers.DecimalField(
    ...
    def validate(self, attrs):
        ...
        qtd_comodos = (
            attrs["quantidade_quartos"]
            + attrs["quantidade_salas"]
            + attrs["quantidade_cozinhas"]
            + attrs["quantidade_banheiros"]
    ...
    def validate_preco(self, preco):
        ...
        valor_total += (
            servico.valor_banheiro * self.initial_data["quantidade_banheiros"]
        )
    ...
    def validate_tempo_atendimento(self, tempo_atendimento):
        ...
        horas_total += (
            servico.horas_banheiro * self.initial_data["quantidade_banheiros"]
        )
    ...
    def validate_data_atendimento(self, data_atendimento):
        if data_atendimento.hour < 6:
            raise serializers.ValidationError(
                "Horário de início não pode ser menor que 6"
            )
        if (data_atendimento.hour + self.initial_data["tempo_atendimento"]) > 22:
            raise serializers.ValidationError(
                "O horário de atendimento não pode passar das 22:00"
            )
        if data_atendimento <= (timezone.now() + timezone.timedelta(hours=48)):
            raise serializers.ValidationError(
                "A data de atendimento não pode ser menor \
    ...
    def get_links(self, obj):
        ...
        if obj.status == 1:
            if usuario.tipo_usuario == 1:
                links.add_post(
                    "pagar_diaria",
                    reverse("pagamento-diaria-list", kwargs={"diaria_id": obj.id}),
                )
        elif obj.status == 2:
            links.add_get(
                "self", reverse("diaria-detail", kwargs={"diaria_id": obj.id})
            )
            data_atendimento = obj.data_atendimento.replace(tzinfo=None)
            if data_atual < data_atendimento:
                links.add_patch(
                    "cancelar_diaria",
                    reverse("cancelar-diaria-detail", kwargs={"diaria_id": obj.id}),
                )
            if usuario.tipo_usuario == 2:
                links.add_post(
                    "candidatar_diaria",
                    reverse(
                        "candidatar-diarista-diaria-list", kwargs={"diaria_id": obj.id}
                    ),
                )
        elif obj.status == 3:
            links.add_get(
            ...
            if usuario.tipo_usuario == 1:
                ...
                if data_atual >= data_atendimento:
                    links.add_patch(
                        "confirmar_diarista",
                        reverse(
                            "confirmar-presenca-diaria-detail",
                            kwargs={"diaria_id": obj.id},
                        ),
                    )
        elif obj.status == 4:
            avaliacoes_diaria = (
                obj.avaliacao_diaria.filter(avaliador__isnull=False) or None
            )
                ...
                for avaliacao in avaliacoes_diaria:
                    if not usuario.id == avaliacao.avaliador.id:
                        links.add_patch(
                            "avaliar_diaria",
                            reverse(
                                "avaliacao-diaria-detail", kwargs={"diaria_id": obj.id}
                            ),
                        )
        else:
            links.add_get(
